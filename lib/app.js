...
app.get('/leaderboard', async(req, res) => {
  ...
  SELECT stats.id, stats.total_score, stats.games_played, users.display_name AS name
  FROM stats
  JOIN users ON stats.user_id=users.id`);
    res.json(data.rows);
  } catch(e) {
    // I would have liked to see unique error messages for each route
...
app.get('/results/:id', async(req, res) => {
  try {
    const data = await client.query(`
    SELECT stats.highest_score, stats.total_score, stats.games_played, users.display_name AS name
    FROM stats
    JOIN users ON stats.user_id=users.id
    WHERE stats.id = $1`,
    [req.params.id]);
